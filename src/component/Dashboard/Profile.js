...
const Profile = () => {
    const { isLoading, error, data:user } = useQuery('repoData', () =>
   ...
   if(isLoading){
	...
	<div className="space-y-4 text-center divide-gray-700">
		<div className="my-2   space-y-1">
			<h2 className="text-xl text-center font-semibold sm:text-2xl">Name : {user.name}</h2>
			<div className=''>
            <p><span className='font-bold'>Collage Name</span> : {user.instituteName}</p>
			<p><span className='font-bold'>Department</span> : {user.Department}</p>
			<p><span className='font-bold'>Address</span> : {user.Address}</p>
			<p><span className='font-bold'>Linkedin</span> : {user.Linkedin}</p>
